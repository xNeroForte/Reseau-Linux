...
🖥️ **VM `proxy.tp3.linux`**: `10.102.1.13`
...
```bash
[user@proxy ~]$ sudo dnf install -y nginx
[user@proxy ~]$ sudo systemctl enable nginx
[user@proxy ~]$ sudo systemctl start nginx
[user@proxy ~]$ sudo ss -laputnr | grep nginx
tcp   LISTEN 0      511                      [::]:80           [::]:*     users:(("nginx",pid=3126,fd=7),("nginx",pid=3125,fd=7))

[user@proxy ~]$ curl localhost
<!doctype html>
<html>
  <head>
    <meta charset='utf-8'>
...
```bash
[user@proxy nginx]$ cat nginx.conf
```

- Les fichiers inclus par le fichier de conf principal sont spécifiés à cette ligne `include /etc/nginx/default.d/*.conf;`
- On crée donc le fichier sample.conf:

```
[user@proxy nginx]$ sudo vi /etc/nginx/default.d/sample.conf
```
